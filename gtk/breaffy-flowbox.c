...
#include "a11y/breaffy-flowboxaccessibleprivate.h"
#include "a11y/breaffy-flowboxaccessible.h"

/* Forward declarations and utilities {{{1 */

static void breaffy_flow_box_update_cursor       (BreaffyFlowBox      *box,
                                              BreaffyFlowBoxChild *child);
static void breaffy_flow_box_select_and_activate (BreaffyFlowBox      *box,
                                              BreaffyFlowBoxChild *child);
static void breaffy_flow_box_update_selection    (BreaffyFlowBox      *box,
                                              BreaffyFlowBoxChild *child,
                                              gboolean         modify,
                                              gboolean         extend);
static void breaffy_flow_box_apply_filter        (BreaffyFlowBox      *box,
                                              BreaffyFlowBoxChild *child);
static void breaffy_flow_box_apply_sort          (BreaffyFlowBox      *box,
                                              BreaffyFlowBoxChild *child);
static gint breaffy_flow_box_sort                (BreaffyFlowBoxChild *a,
                                              BreaffyFlowBoxChild *b,
                                              BreaffyFlowBox      *box);

static void breaffy_flow_box_bound_model_changed (GListModel *list,
                                              guint       position,
                                              guint       removed,
                                              guint       added,
                                              gpointer    user_data);

static void breaffy_flow_box_set_accept_unpaired_release (BreaffyFlowBox *box,
                                                      gboolean    accept);

static void breaffy_flow_box_check_model_compat  (BreaffyFlowBox *box);
...
typedef struct _BreaffyFlowBoxChildPrivate BreaffyFlowBoxChildPrivate;
struct _BreaffyFlowBoxChildPrivate
{
  GSequenceIter *iter;
  gboolean       selected;
};

#define CHILD_PRIV(child) ((BreaffyFlowBoxChildPrivate*)breaffy_flow_box_child_get_instance_private ((BreaffyFlowBoxChild*)(child)))

G_DEFINE_TYPE_WITH_PRIVATE (BreaffyFlowBoxChild, breaffy_flow_box_child, GTK_TYPE_BIN)

/* Internal API {{{2 */

static BreaffyFlowBox *
breaffy_flow_box_child_get_box (BreaffyFlowBoxChild *child)
{
  GtkWidget *parent;

  parent = gtk_widget_get_parent (GTK_WIDGET (child));
  if (parent && BREAFFY_IS_FLOW_BOX (parent))
    return BREAFFY_FLOW_BOX (parent);

  return NULL;
}

static void
breaffy_flow_box_child_set_focus (BreaffyFlowBoxChild *child)
{
  BreaffyFlowBox *box = breaffy_flow_box_child_get_box (child);
  gboolean modify;
  gboolean extend;
  ...
  if (modify)
    breaffy_flow_box_update_cursor (box, child);
  else
    breaffy_flow_box_update_selection (box, child, FALSE, FALSE);
}

/* GtkWidget implementation {{{2 */

static gboolean
breaffy_flow_box_child_focus (GtkWidget        *widget,
      ...
      if (child)
        {
          if (gtk_widget_child_focus (child, direction))
            {
              BreaffyFlowBox *box;
              box = breaffy_flow_box_child_get_box (BREAFFY_FLOW_BOX_CHILD (widget));
              if (box)
                breaffy_flow_box_update_cursor (box, BREAFFY_FLOW_BOX_CHILD (widget));
              return TRUE;
            }
        }
      return FALSE;
  ...
  if (had_focus)
  ...
  else if (gtk_widget_get_focus_child (widget) != NULL)
    {
      ...
      if (direction == GTK_DIR_LEFT || direction == GTK_DIR_TAB_BACKWARD)
        {
          breaffy_flow_box_child_set_focus (BREAFFY_FLOW_BOX_CHILD (widget));
          return TRUE;
        }
...
static void
breaffy_flow_box_child_activate (BreaffyFlowBoxChild *child)
{
  BreaffyFlowBox *box;

  box = breaffy_flow_box_child_get_box (child);
  if (box)
    breaffy_flow_box_select_and_activate (box, child);
}

/* Size allocation {{{3 */

static GtkSizeRequestMode
breaffy_flow_box_child_get_request_mode (GtkWidget *widget)
{
  BreaffyFlowBox *box;
...
static void
breaffy_flow_box_child_measure (GtkWidget     *widget,
...
static void
breaffy_flow_box_child_size_allocate (GtkWidget           *widget,
...
static void
breaffy_flow_box_child_class_init (BreaffyFlowBoxChildClass *class)
{
  GObjectClass *object_class = G_OBJECT_CLASS (class);
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (class);

  widget_class->get_request_mode = breaffy_flow_box_child_get_request_mode;
  widget_class->measure = breaffy_flow_box_child_measure;
  widget_class->size_allocate = breaffy_flow_box_child_size_allocate;
  widget_class->focus = breaffy_flow_box_child_focus;

  class->activate = breaffy_flow_box_child_activate;

  /**
   * BreaffyFlowBoxChild::activate:
   * @child: The child on which the signal is emitted
   *
   * The ::activate signal is emitted when the user activates
   * a child widget in a #BreaffyFlowBox, either by clicking or
  ...
  child_signals[CHILD_ACTIVATE] =
    g_signal_new (I_("activate"),
                  ...
                  G_STRUCT_OFFSET (BreaffyFlowBoxChildClass, activate),
                  NULL, NULL,
...
static void
breaffy_flow_box_child_init (BreaffyFlowBoxChild *child)
{
...
/**
 * breaffy_flow_box_child_new:
 *
 * Creates a new #BreaffyFlowBoxChild, to be used as a child
 * of a #BreaffyFlowBox.
 *
 * Returns: a new #BreaffyFlowBoxChild
 */
GtkWidget *
breaffy_flow_box_child_new (void)
{
  return g_object_new (BREAFFY_TYPE_FLOW_BOX_CHILD, NULL);
}

/**
 * breaffy_flow_box_child_get_index:
 * @child: a #BreaffyFlowBoxChild
 *
 * Gets the current index of the @child in its #BreaffyFlowBox container.
 *
 * Returns: the index of the @child, or -1 if the @child is not
 *     in a flow box.
 */
gint
breaffy_flow_box_child_get_index (BreaffyFlowBoxChild *child)
{
  BreaffyFlowBoxChildPrivate *priv;

  g_return_val_if_fail (BREAFFY_IS_FLOW_BOX_CHILD (child), -1);
...
/**
 * breaffy_flow_box_child_is_selected:
 * @child: a #BreaffyFlowBoxChild
 *
 * Returns whether the @child is currently selected in its
 * #BreaffyFlowBox container.
 *
 * Returns: %TRUE if @child is selected
 */
gboolean
breaffy_flow_box_child_is_selected (BreaffyFlowBoxChild *child)
{
  g_return_val_if_fail (BREAFFY_IS_FLOW_BOX_CHILD (child), FALSE);

  return CHILD_PRIV (child)->selected;
}

/**
 * breaffy_flow_box_child_changed:
 * @child: a #BreaffyFlowBoxChild
 *
 ...
 * breaffy_flow_box_child_changed() on the first child, the sort function
 ...
 * alternative is to call breaffy_flow_box_invalidate_sort() on any
 * model change, but that is more expensive.
 */
void
breaffy_flow_box_child_changed (BreaffyFlowBoxChild *child)
{
  BreaffyFlowBox *box;

  g_return_if_fail (BREAFFY_IS_FLOW_BOX_CHILD (child));
  ...
  breaffy_flow_box_apply_sort (box, child);
  breaffy_flow_box_apply_filter (box, child);
}
...
typedef struct _BreaffyFlowBoxPrivate BreaffyFlowBoxPrivate;
struct _BreaffyFlowBoxPrivate {
  GtkOrientation    orientation;
  ...
  BreaffyFlowBoxChild  *cursor_child;
  BreaffyFlowBoxChild  *selected_child;

  BreaffyFlowBoxChild  *active_child;
  ...
  BreaffyFlowBoxChild   *rubberband_first;
  BreaffyFlowBoxChild   *rubberband_last;
...
#define BOX_PRIV(box) ((BreaffyFlowBoxPrivate*)breaffy_flow_box_get_instance_private ((BreaffyFlowBox*)(box)))

G_DEFINE_TYPE_WITH_CODE (BreaffyFlowBox, breaffy_flow_box, GTK_TYPE_CONTAINER,
                         G_ADD_PRIVATE (BreaffyFlowBox)
...
static gint
get_visible_children (BreaffyFlowBox *box)
{
  GSequenceIter *iter;
  gint i = 0;
...
static void
breaffy_flow_box_apply_filter (BreaffyFlowBox      *box,
                           BreaffyFlowBoxChild *child)
{
  BreaffyFlowBoxPrivate *priv = BOX_PRIV (box);
  gboolean do_show;
...
static void
breaffy_flow_box_apply_filter_all (BreaffyFlowBox *box)
{
  GSequenceIter *iter;
...
static void
breaffy_flow_box_apply_sort (BreaffyFlowBox      *box,
                         BreaffyFlowBoxChild *child)
{
  if (BOX_PRIV (box)->sort_func != NULL)
    {
      g_sequence_sort_changed (CHILD_PRIV (child)->iter,
                               (GCompareDataFunc)breaffy_flow_box_sort, box);
      gtk_widget_queue_resize (GTK_WIDGET (box));
    }
}
...
static gboolean
breaffy_flow_box_child_set_selected (BreaffyFlowBoxChild *child,
                                 gboolean         selected)
{
...
static gboolean
breaffy_flow_box_unselect_all_internal (BreaffyFlowBox *box)
{
  BreaffyFlowBoxChild *child;
  GSequenceIter *iter;
  ...
  for (iter = g_sequence_get_begin_iter (BOX_PRIV (box)->children);
    ...
    {
      child = g_sequence_get (iter);
      dirty |= breaffy_flow_box_child_set_selected (child, FALSE);
    }
...
static void
breaffy_flow_box_unselect_child_internal (BreaffyFlowBox      *box,
                                      BreaffyFlowBoxChild *child)
{
  ...
  if (BOX_PRIV (box)->selection_mode == GTK_SELECTION_NONE)
    ...
    breaffy_flow_box_unselect_all_internal (box);
  else
    breaffy_flow_box_child_set_selected (child, FALSE);

  g_signal_emit (box, signals[SELECTED_CHILDREN_CHANGED], 0);
}

static void
breaffy_flow_box_update_cursor (BreaffyFlowBox      *box,
                            BreaffyFlowBoxChild *child)
{
  BOX_PRIV (box)->cursor_child = child;
  gtk_widget_grab_focus (GTK_WIDGET (child));
  _breaffy_flow_box_accessible_update_cursor (GTK_WIDGET (box), GTK_WIDGET (child));
}

static void
breaffy_flow_box_select_child_internal (BreaffyFlowBox      *box,
                                    BreaffyFlowBoxChild *child)
{
  if (CHILD_PRIV (child)->selected)
    return;
  ...
  breaffy_flow_box_child_set_selected (child, TRUE);
...
static void
breaffy_flow_box_select_all_between (BreaffyFlowBox      *box,
                                 BreaffyFlowBoxChild *child1,
  ...
  for (iter = iter1;
       ...
       iter = g_sequence_iter_next (iter))
    {
      ...
      if (child_is_visible (child))
        {
          if (modify)
            breaffy_flow_box_child_set_selected (BREAFFY_FLOW_BOX_CHILD (child), !CHILD_PRIV (child)->selected);
          else
            breaffy_flow_box_child_set_selected (BREAFFY_FLOW_BOX_CHILD (child), TRUE);
	}
...
static void
breaffy_flow_box_update_selection (BreaffyFlowBox      *box,
                               BreaffyFlowBoxChild *child,
                               gboolean         modify,
                               gboolean         extend)
{
  ...
  if (priv->selection_mode == GTK_SELECTION_BROWSE)
  ...
  else if (priv->selection_mode == GTK_SELECTION_SINGLE)
    {
      ...
      breaffy_flow_box_unselect_all_internal (box);
      breaffy_flow_box_child_set_selected (child, modify ? !was_selected : TRUE);
      ...
      if (extend)
        {
          ...
          if (priv->selected_child == NULL)
            ...
            breaffy_flow_box_select_all_between (box, priv->selected_child, child, FALSE);
        }
      else
        {
          if (modify)
            {
              breaffy_flow_box_child_set_selected (child, !CHILD_PRIV (child)->selected);
            }
          else
            {
              breaffy_flow_box_unselect_all_internal (box);
...
static void
breaffy_flow_box_select_and_activate (BreaffyFlowBox      *box,
                                  BreaffyFlowBoxChild *child)
{
  if (child != NULL)
    {
      breaffy_flow_box_select_child_internal (box, child);
      breaffy_flow_box_update_cursor (box, child);
...
breaffy_flow_box_get_previous_focusable (BreaffyFlowBox    *box,
                                     GSequenceIter *iter)
{
  BreaffyFlowBoxChild *child;
...
static GSequenceIter *
breaffy_flow_box_get_next_focusable (BreaffyFlowBox    *box,
                                 GSequenceIter *iter)
{
  BreaffyFlowBoxChild *child;
...
static GSequenceIter *
breaffy_flow_box_get_first_focusable (BreaffyFlowBox *box)
{
  GSequenceIter *iter;
  BreaffyFlowBoxChild *child;
  ...
  return breaffy_flow_box_get_next_focusable (box, iter);
}

static GSequenceIter *
breaffy_flow_box_get_last_focusable (BreaffyFlowBox *box)
{
  GSequenceIter *iter;

  iter = g_sequence_get_end_iter (BOX_PRIV (box)->children);
  return breaffy_flow_box_get_previous_focusable (box, iter);
}


static GSequenceIter *
breaffy_flow_box_get_above_focusable (BreaffyFlowBox    *box,
                                  GSequenceIter *iter)
{
  BreaffyFlowBoxChild *child = NULL;
  gint i;
...
static GSequenceIter *
breaffy_flow_box_get_below_focusable (BreaffyFlowBox    *box,
                                  GSequenceIter *iter)
{
  BreaffyFlowBoxChild *child = NULL;
  gint i;
 ...
 */
static void
get_max_item_size (BreaffyFlowBox     *box,
                   GtkOrientation  orientation,
                   gint           *min_size,
                   gint           *nat_size)
{
 ...
 */
static void
get_largest_size_for_opposing_orientation (BreaffyFlowBox     *box,
                                           GtkOrientation  orientation,
                                           gint            item_size,
                                           gint           *min_item_size,
                                           gint           *nat_item_size)
 ...
 */
static GSequenceIter *
get_largest_size_for_line_in_opposing_orientation (BreaffyFlowBox       *box,
                                                   GtkOrientation    orientation,
...
static gint
gather_aligned_item_requests (BreaffyFlowBox       *box,
                              GtkOrientation    orientation,
                              gint              line_length,
                              gint              item_spacing,
                              gint              n_children,
...
fit_aligned_item_requests (BreaffyFlowBox     *box,
                           GtkOrientation  orientation,
                           gint            avail_size,
                           gint            item_spacing,
...
static void
breaffy_flow_box_size_allocate (GtkWidget           *widget,
                            const GtkAllocation *allocation,
                            int                  baseline)
{
  BreaffyFlowBox *box = BREAFFY_FLOW_BOX (widget);
  BreaffyFlowBoxPrivate  *priv = BOX_PRIV (box);
...
static GtkSizeRequestMode
breaffy_flow_box_get_request_mode (GtkWidget *widget)
{
  BreaffyFlowBox *box = BREAFFY_FLOW_BOX (widget);
...
static void
get_largest_aligned_line_length (BreaffyFlowBox     *box,
                                 GtkOrientation  orientation,
                                 gint            line_length,
                                 gint           *min_size,
                                 gint           *nat_size)
...
static void
breaffy_flow_box_measure (GtkWidget      *widget,
                      GtkOrientation  orientation,
                      int             for_size,
                      int            *minimum,
                      int            *natural,
      ...
      if (for_size < 0)
        {
          ...
          if (priv->orientation == GTK_ORIENTATION_HORIZONTAL)
          ...
          else /* GTK_ORIENTATION_VERTICAL */
            {
              ...
              breaffy_flow_box_measure (widget,
...
static void
breaffy_flow_box_snapshot (GtkWidget   *widget,
                       GtkSnapshot *snapshot)
{
  ...
  GTK_WIDGET_CLASS (breaffy_flow_box_parent_class)->snapshot (widget, snapshot);
...
static void
remove_autoscroll (BreaffyFlowBox *box)
{
  BreaffyFlowBoxPrivate *priv = BOX_PRIV (box);
...
autoscroll_cb (GtkWidget     *widget,
               ...
               gpointer       data)
{
  BreaffyFlowBox *box = BREAFFY_FLOW_BOX (data);
  BreaffyFlowBoxPrivate *priv = BOX_PRIV (box);
  ...
  if (priv->rubberband_select)
    {
      ...
      child = breaffy_flow_box_get_child_at_pos (box, x, y);
...
static void
add_autoscroll (BreaffyFlowBox *box)
{
  BreaffyFlowBoxPrivate *priv = BOX_PRIV (box);
...
static gboolean
get_view_rect (BreaffyFlowBox   *box,
               GdkRectangle *rect)
{
  BreaffyFlowBoxPrivate *priv = BOX_PRIV (box);
  GtkWidget *parent;
...
static void
update_autoscroll_mode (BreaffyFlowBox *box,
                        gint        x,
                        gint        y)
{
  BreaffyFlowBoxPrivate *priv = BOX_PRIV (box);
...
static void
breaffy_flow_box_drag_gesture_update (GtkGestureDrag *gesture,
                                  gdouble         offset_x,
                                  gdouble         offset_y,
                                  BreaffyFlowBox     *box)
{
  ...
  if (!priv->rubberband_select &&
      (offset_x * offset_x) + (offset_y * offset_y) > RUBBERBAND_START_DISTANCE * RUBBERBAND_START_DISTANCE)
    {
      ...
      priv->rubberband_first = breaffy_flow_box_get_child_at_pos (box, start_x, start_y);
      ...
      if (priv->rubberband_first)
        breaffy_flow_box_update_cursor (box, priv->rubberband_first);
      gtk_gesture_set_state (GTK_GESTURE (gesture), GTK_EVENT_SEQUENCE_CLAIMED);
    }

  if (priv->rubberband_select)
    {
      child = breaffy_flow_box_get_child_at_pos (box, start_x + offset_x,
                                              start_y + offset_y);
...
static void
breaffy_flow_box_multipress_gesture_pressed (GtkGestureMultiPress *gesture,
                                         guint                 n_press,
                                         gdouble               x,
                                         gdouble               y,
                                         BreaffyFlowBox           *box)
...
static void
breaffy_flow_box_multipress_unpaired_release (GtkGestureMultiPress *gesture,
                                          gdouble               x,
                                          gdouble               y,
                                          guint                 button,
                                          GdkEventSequence     *sequence,
...
static void
breaffy_flow_box_multipress_gesture_released (GtkGestureMultiPress *gesture,
                                          guint                 n_press,
                                          gdouble               x,
                                          gdouble               y,
                                          BreaffyFlowBox           *box)
{
  ...
  if (priv->active_child != NULL &&
      priv->active_child == breaffy_flow_box_get_child_at_pos (box, x, y))
    {
      if (priv->activate_on_single_click)
        breaffy_flow_box_select_and_activate (box, priv->active_child);
      else
        {
          GdkEventSequence *sequence;
          ...
          breaffy_flow_box_update_selection (box, priv->active_child, modify, extend);
        }
    }
}

static void
breaffy_flow_box_multipress_gesture_stopped (GtkGestureMultiPress *gesture,
                                         BreaffyFlowBox           *box)
{
  BreaffyFlowBoxPrivate *priv = BOX_PRIV (box);
...
static void
breaffy_flow_box_drag_gesture_begin (GtkGestureDrag *gesture,
                                 gdouble         start_x,
                                 gdouble         start_y,
                                 GtkWidget      *widget)
{
  BreaffyFlowBoxPrivate *priv = BOX_PRIV (widget);
...
static void
breaffy_flow_box_stop_rubberband (BreaffyFlowBox *box)
{
  BreaffyFlowBoxPrivate *priv = BOX_PRIV (box);
...
static void
breaffy_flow_box_drag_gesture_end (GtkGestureDrag *gesture,
                               gdouble         offset_x,
                               gdouble         offset_y,
                               BreaffyFlowBox     *box)
{
  ...
  if (gtk_gesture_handles_sequence (GTK_GESTURE (gesture), sequence))
    {
      ...
      if (priv->rubberband_first && priv->rubberband_last)
        breaffy_flow_box_select_all_between (box, priv->rubberband_first, priv->rubberband_last, priv->rubberband_modify);

      breaffy_flow_box_stop_rubberband (box);
...
static gboolean
breaffy_flow_box_key_controller_key_pressed (GtkEventControllerKey *controller,
                                         guint                  keyval,
                                         guint                  keycode,
...
static void
breaffy_flow_box_unmap (GtkWidget *widget)
{
  BreaffyFlowBox *box = BREAFFY_FLOW_BOX (widget);

  remove_autoscroll (box);

  GTK_WIDGET_CLASS (breaffy_flow_box_parent_class)->unmap (widget);
}

/* GtkContainer implementation {{{2 */

static void
breaffy_flow_box_add (GtkContainer *container,
                  GtkWidget    *child)
{
  breaffy_flow_box_insert (BREAFFY_FLOW_BOX (container), child, -1);
}

static void
breaffy_flow_box_remove (GtkContainer *container,
                     GtkWidget    *widget)
{
  BreaffyFlowBox *box = BREAFFY_FLOW_BOX (container);
  BreaffyFlowBoxPrivate *priv = BOX_PRIV (box);
  gboolean was_visible;
  gboolean was_selected;
  BreaffyFlowBoxChild *child;

  if (BREAFFY_IS_FLOW_BOX_CHILD (widget))
    child = BREAFFY_FLOW_BOX_CHILD (widget);
  else
    {
      child = (BreaffyFlowBoxChild*)gtk_widget_get_parent (widget);
      if (!BREAFFY_IS_FLOW_BOX_CHILD (child))
        {
...
static void
breaffy_flow_box_forall (GtkContainer *container,
...
static GType
breaffy_flow_box_child_type (GtkContainer *container)
{
  return BREAFFY_TYPE_FLOW_BOX_CHILD;
}

/* Keynav {{{2 */

static gboolean
breaffy_flow_box_focus (GtkWidget        *widget,
                    GtkDirectionType  direction)
{
  BreaffyFlowBox *box = BREAFFY_FLOW_BOX (widget);
  GtkWidget *focus_child;
  GSequenceIter *iter;
  BreaffyFlowBoxChild *next_focus_child;

  /* Without "can-focus" flag fall back to the default behavior immediately */
  if (!gtk_widget_get_can_focus (widget))
    {
      return GTK_WIDGET_CLASS (breaffy_flow_box_parent_class)->focus (widget, direction);
    }
  ...
  if (focus_child != NULL)
    {
      ...
      if (direction == GTK_DIR_LEFT || direction == GTK_DIR_TAB_BACKWARD)
        iter = breaffy_flow_box_get_previous_focusable (box, iter);
      else if (direction == GTK_DIR_RIGHT || direction == GTK_DIR_TAB_FORWARD)
        iter = breaffy_flow_box_get_next_focusable (box, iter);
      else if (direction == GTK_DIR_UP)
        iter = breaffy_flow_box_get_above_focusable (box, iter);
      else if (direction == GTK_DIR_DOWN)
        iter = breaffy_flow_box_get_below_focusable (box, iter);
      ...
      if (BOX_PRIV (box)->selected_child)
      ...
      else
        {
          if (direction == GTK_DIR_UP || direction == GTK_DIR_TAB_BACKWARD)
            iter = breaffy_flow_box_get_last_focusable (box);
          else
            iter = breaffy_flow_box_get_first_focusable (box);
...
static void
breaffy_flow_box_add_move_binding (GtkBindingSet   *binding_set,
                               guint            keyval,
...
static void
breaffy_flow_box_activate_cursor_child (BreaffyFlowBox *box)
{
  breaffy_flow_box_select_and_activate (box, BOX_PRIV (box)->cursor_child);
}

static void
breaffy_flow_box_toggle_cursor_child (BreaffyFlowBox *box)
{
  BreaffyFlowBoxPrivate *priv = BOX_PRIV (box);
  ...
  if ((priv->selection_mode == GTK_SELECTION_SINGLE ||
    ...
    breaffy_flow_box_unselect_child_internal (box, priv->cursor_child);
  else
    breaffy_flow_box_select_and_activate (box, priv->cursor_child);
}

static gboolean
breaffy_flow_box_move_cursor (BreaffyFlowBox      *box,
                          GtkMovementStep  step,
                          gint             count)
{
  BreaffyFlowBoxPrivate *priv = BOX_PRIV (box);
  ...
  gboolean extend;
  BreaffyFlowBoxChild *child;
  BreaffyFlowBoxChild *prev;
  BreaffyFlowBoxChild *next;
  ...
  breaffy_flow_box_update_cursor (box, child);
  if (!modify)
    breaffy_flow_box_update_selection (box, child, FALSE, extend);
  return TRUE;
}
...
static void
breaffy_flow_box_selected_children_changed (BreaffyFlowBox *box)
{
  _breaffy_flow_box_accessible_selection_changed (GTK_WIDGET (box));
}

/* GObject implementation {{{2 */

static void
breaffy_flow_box_get_property (GObject    *object,
                           guint       prop_id,
                           GValue     *value,
                           GParamSpec *pspec)
{
  BreaffyFlowBox *box = BREAFFY_FLOW_BOX (object);
  BreaffyFlowBoxPrivate *priv = BOX_PRIV (box);
...
static void
breaffy_flow_box_set_property (GObject      *object,
                           guint         prop_id,
                           const GValue *value,
                           GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_ORIENTATION:
      ...
      break;
    case PROP_HOMOGENEOUS:
      breaffy_flow_box_set_homogeneous (box, g_value_get_boolean (value));
      break;
    case PROP_COLUMN_SPACING:
      breaffy_flow_box_set_column_spacing (box, g_value_get_uint (value));
      break;
    case PROP_ROW_SPACING:
      breaffy_flow_box_set_row_spacing (box, g_value_get_uint (value));
      break;
    case PROP_MIN_CHILDREN_PER_LINE:
      breaffy_flow_box_set_min_children_per_line (box, g_value_get_uint (value));
      break;
    case PROP_MAX_CHILDREN_PER_LINE:
      breaffy_flow_box_set_max_children_per_line (box, g_value_get_uint (value));
      break;
    case PROP_SELECTION_MODE:
      breaffy_flow_box_set_selection_mode (box, g_value_get_enum (value));
      break;
    case PROP_ACTIVATE_ON_SINGLE_CLICK:
      breaffy_flow_box_set_activate_on_single_click (box, g_value_get_boolean (value));
      break;
    case PROP_ACCEPT_UNPAIRED_RELEASE:
      breaffy_flow_box_set_accept_unpaired_release (box, g_value_get_boolean (value));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
...
static void
breaffy_flow_box_finalize (GObject *obj)
{
  BreaffyFlowBoxPrivate *priv = BOX_PRIV (obj);
  ...
  if (priv->bound_model)
    {
      ...
      g_signal_handlers_disconnect_by_func (priv->bound_model, breaffy_flow_box_bound_model_changed, obj);
      g_clear_object (&priv->bound_model);
    }

  G_OBJECT_CLASS (breaffy_flow_box_parent_class)->finalize (obj);
}

static void
breaffy_flow_box_class_init (BreaffyFlowBoxClass *class)
{
  GObjectClass      *object_class = G_OBJECT_CLASS (class);
  GtkWidgetClass    *widget_class = GTK_WIDGET_CLASS (class);
  ...
  object_class->finalize = breaffy_flow_box_finalize;
  object_class->get_property = breaffy_flow_box_get_property;
  object_class->set_property = breaffy_flow_box_set_property;

  widget_class->size_allocate = breaffy_flow_box_size_allocate;
  widget_class->unmap = breaffy_flow_box_unmap;
  widget_class->focus = breaffy_flow_box_focus;
  widget_class->snapshot = breaffy_flow_box_snapshot;
  widget_class->get_request_mode = breaffy_flow_box_get_request_mode;
  widget_class->measure = breaffy_flow_box_measure;

  container_class->add = breaffy_flow_box_add;
  container_class->remove = breaffy_flow_box_remove;
  container_class->forall = breaffy_flow_box_forall;
  container_class->child_type = breaffy_flow_box_child_type;

  class->activate_cursor_child = breaffy_flow_box_activate_cursor_child;
  class->toggle_cursor_child = breaffy_flow_box_toggle_cursor_child;
  class->move_cursor = breaffy_flow_box_move_cursor;
  class->select_all = breaffy_flow_box_select_all;
  class->unselect_all = breaffy_flow_box_unselect_all;
  class->selected_children_changed = breaffy_flow_box_selected_children_changed;

  g_object_class_override_property (object_class, PROP_ORIENTATION, "orientation");

  /**
   * BreaffyFlowBox:selection-mode:
   *
  ...
  /**
   * BreaffyFlowBox:activate-on-single-click:
   *
  ...
  /**
   * BreaffyFlowBox:min-children-per-line:
   *
  ...
  /**
   * BreaffyFlowBox:max-children-per-line:
   *
  ...
  /**
   * BreaffyFlowBox::child-activated:
   * @box: the #BreaffyFlowBox on which the signal is emitted
  ...
  signals[CHILD_ACTIVATED] = g_signal_new (I_("child-activated"),
                                           BREAFFY_TYPE_FLOW_BOX,
                                           G_SIGNAL_RUN_LAST,
                                           G_STRUCT_OFFSET (BreaffyFlowBoxClass, child_activated),
                                           NULL, NULL,
                                           g_cclosure_marshal_VOID__OBJECT,
                                           G_TYPE_NONE, 1,
                                           BREAFFY_TYPE_FLOW_BOX_CHILD);

  /**
   * BreaffyFlowBox::selected-children-changed:
   * @box: the #BreaffyFlowBox on wich the signal is emitted
   *
   * The ::selected-children-changed signal is emitted when the
   * set of selected children changes.
   *
   * Use breaffy_flow_box_selected_foreach() or
   * breaffy_flow_box_get_selected_children() to obtain the
   * selected children.
   */
  signals[SELECTED_CHILDREN_CHANGED] = g_signal_new (I_("selected-children-changed"),
                                                     BREAFFY_TYPE_FLOW_BOX,
                                                     G_SIGNAL_RUN_FIRST,
                                                     G_STRUCT_OFFSET (BreaffyFlowBoxClass, selected_children_changed),
                                                     NULL, NULL,
                                                     g_cclosure_marshal_VOID__VOID,
                                                     G_TYPE_NONE, 0);

  /**
   * BreaffyFlowBox::activate-cursor-child:
   * @box: the #BreaffyFlowBox on which the signal is emitted
   *
  ...
  signals[ACTIVATE_CURSOR_CHILD] = g_signal_new (I_("activate-cursor-child"),
                                                 ...
                                                 G_STRUCT_OFFSET (BreaffyFlowBoxClass, activate_cursor_child),
                                                 NULL, NULL,
                                                 g_cclosure_marshal_VOID__VOID,
                                                 G_TYPE_NONE, 0);

  /**
   * BreaffyFlowBox::toggle-cursor-child:
   * @box: the #BreaffyFlowBox on which the signal is emitted
   *
  ...
  signals[TOGGLE_CURSOR_CHILD] = g_signal_new (I_("toggle-cursor-child"),
                                               BREAFFY_TYPE_FLOW_BOX,
                                               G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                                               G_STRUCT_OFFSET (BreaffyFlowBoxClass, toggle_cursor_child),
                                               NULL, NULL,
                                               g_cclosure_marshal_VOID__VOID,
                                               G_TYPE_NONE, 0);

  /**
   * BreaffyFlowBox::move-cursor:
   * @box: the #BreaffyFlowBox on which the signal is emitted
  ...
  signals[MOVE_CURSOR] = g_signal_new (I_("move-cursor"),
                                       BREAFFY_TYPE_FLOW_BOX,
                                       G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                                       G_STRUCT_OFFSET (BreaffyFlowBoxClass, move_cursor),
                                       NULL, NULL,
                                       _gtk_marshal_BOOLEAN__ENUM_INT,
                                       G_TYPE_BOOLEAN, 2,
                                       GTK_TYPE_MOVEMENT_STEP, G_TYPE_INT);
  /**
   * BreaffyFlowBox::select-all:
   * @box: the #BreaffyFlowBox on which the signal is emitted
   *
  ...
  signals[SELECT_ALL] = g_signal_new (I_("select-all"),
                                      BREAFFY_TYPE_FLOW_BOX,
                                      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                                      G_STRUCT_OFFSET (BreaffyFlowBoxClass, select_all),
                                      NULL, NULL,
                                      g_cclosure_marshal_VOID__VOID,
                                      G_TYPE_NONE, 0);

  /**
   * BreaffyFlowBox::unselect-all:
   * @box: the #BreaffyFlowBox on which the signal is emitted
   *
  ...
  signals[UNSELECT_ALL] = g_signal_new (I_("unselect-all"),
                                        BREAFFY_TYPE_FLOW_BOX,
                                        G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                                        G_STRUCT_OFFSET (BreaffyFlowBoxClass, unselect_all),
                                        NULL, NULL,
                                        g_cclosure_marshal_VOID__VOID,
                                        G_TYPE_NONE, 0);
  ...
  breaffy_flow_box_add_move_binding (binding_set, GDK_KEY_Home, 0,
                                 GTK_MOVEMENT_BUFFER_ENDS, -1);
  breaffy_flow_box_add_move_binding (binding_set, GDK_KEY_KP_Home, 0,
                                 GTK_MOVEMENT_BUFFER_ENDS, -1);
  breaffy_flow_box_add_move_binding (binding_set, GDK_KEY_End, 0,
                                 GTK_MOVEMENT_BUFFER_ENDS, 1);
  breaffy_flow_box_add_move_binding (binding_set, GDK_KEY_KP_End, 0,
                                 GTK_MOVEMENT_BUFFER_ENDS, 1);
  breaffy_flow_box_add_move_binding (binding_set, GDK_KEY_Up, 0,
                                 GTK_MOVEMENT_DISPLAY_LINES, -1);
  breaffy_flow_box_add_move_binding (binding_set, GDK_KEY_KP_Up, 0,
                                 GTK_MOVEMENT_DISPLAY_LINES, -1);
  breaffy_flow_box_add_move_binding (binding_set, GDK_KEY_Down, 0,
                                 GTK_MOVEMENT_DISPLAY_LINES, 1);
  breaffy_flow_box_add_move_binding (binding_set, GDK_KEY_KP_Down, 0,
                                 GTK_MOVEMENT_DISPLAY_LINES, 1);
  breaffy_flow_box_add_move_binding (binding_set, GDK_KEY_Page_Up, 0,
                                 GTK_MOVEMENT_PAGES, -1);
  breaffy_flow_box_add_move_binding (binding_set, GDK_KEY_KP_Page_Up, 0,
                                 GTK_MOVEMENT_PAGES, -1);
  breaffy_flow_box_add_move_binding (binding_set, GDK_KEY_Page_Down, 0,
                                 GTK_MOVEMENT_PAGES, 1);
  breaffy_flow_box_add_move_binding (binding_set, GDK_KEY_KP_Page_Down, 0,
                                 GTK_MOVEMENT_PAGES, 1);

  breaffy_flow_box_add_move_binding (binding_set, GDK_KEY_Right, 0,
                                 GTK_MOVEMENT_VISUAL_POSITIONS, 1);
  breaffy_flow_box_add_move_binding (binding_set, GDK_KEY_KP_Right, 0,
                                 GTK_MOVEMENT_VISUAL_POSITIONS, 1);
  breaffy_flow_box_add_move_binding (binding_set, GDK_KEY_Left, 0,
                                 GTK_MOVEMENT_VISUAL_POSITIONS, -1);
  breaffy_flow_box_add_move_binding (binding_set, GDK_KEY_KP_Left, 0,
                                 GTK_MOVEMENT_VISUAL_POSITIONS, -1);
  ...
  gtk_widget_class_set_accessible_type (widget_class, BREAFFY_TYPE_FLOW_BOX_ACCESSIBLE);
  gtk_widget_class_set_css_name (widget_class, I_("flowbox"));
}

static void
breaffy_flow_box_init (BreaffyFlowBox *box)
{
  BreaffyFlowBoxPrivate *priv = BOX_PRIV (box);
  ...
  g_signal_connect (gesture, "pressed",
                    G_CALLBACK (breaffy_flow_box_multipress_gesture_pressed), box);
  g_signal_connect (gesture, "released",
                    G_CALLBACK (breaffy_flow_box_multipress_gesture_released), box);
  g_signal_connect (gesture, "stopped",
                    G_CALLBACK (breaffy_flow_box_multipress_gesture_stopped), box);
  g_signal_connect (gesture, "unpaired-release",
                    G_CALLBACK (breaffy_flow_box_multipress_unpaired_release), box);
  ...
  g_signal_connect (priv->drag_gesture, "drag-begin",
                    G_CALLBACK (breaffy_flow_box_drag_gesture_begin), box);
  g_signal_connect (priv->drag_gesture, "drag-update",
                    G_CALLBACK (breaffy_flow_box_drag_gesture_update), box);
  g_signal_connect (priv->drag_gesture, "drag-end",
                    G_CALLBACK (breaffy_flow_box_drag_gesture_end), box);
  gtk_widget_add_controller (GTK_WIDGET (box), GTK_EVENT_CONTROLLER (priv->drag_gesture));

  controller = gtk_event_controller_key_new ();
  g_signal_connect (controller, "key-pressed",
                    G_CALLBACK (breaffy_flow_box_key_controller_key_pressed), box);
  gtk_widget_add_controller (GTK_WIDGET (box), controller);
}

static void
breaffy_flow_box_bound_model_changed (GListModel *list,
                                  guint       position,
                                  guint       removed,
                                  guint       added,
                                  gpointer    user_data)
{
  BreaffyFlowBox *box = user_data;
  BreaffyFlowBoxPrivate *priv = BOX_PRIV (box);
  gint i;
  ...
  while (removed--)
    {
      ...
      child = breaffy_flow_box_get_child_at_index (box, position);
  ...
  for (i = 0; i < added; i++)
    {
      ...
      breaffy_flow_box_insert (box, widget, position + i);
...
/**
 * breaffy_flow_box_new:
 *
 * Creates a BreaffyFlowBox.
 *
 * Returns: a new #BreaffyFlowBox container
 */
GtkWidget *
breaffy_flow_box_new (void)
{
  return (GtkWidget *)g_object_new (BREAFFY_TYPE_FLOW_BOX, NULL);
}

static void
breaffy_flow_box_insert_css_node (BreaffyFlowBox    *box,
                              GtkWidget     *child,
                              GSequenceIter *iter)
{
...
/**
 * breaffy_flow_box_insert:
 * @box: a #BreaffyFlowBox
 ...
 */
void
breaffy_flow_box_insert (BreaffyFlowBox *box,
                     GtkWidget  *widget,
                     gint        position)
{
  BreaffyFlowBoxPrivate *priv;
  BreaffyFlowBoxChild *child;
  GSequenceIter *iter;

  g_return_if_fail (BREAFFY_IS_FLOW_BOX (box));
  ...
  if (BREAFFY_IS_FLOW_BOX_CHILD (widget))
    ...
    {
      child = BREAFFY_FLOW_BOX_CHILD (breaffy_flow_box_child_new ());
  ...
  breaffy_flow_box_insert_css_node (box, GTK_WIDGET (child), iter);
...
/**
 * breaffy_flow_box_get_child_at_index:
 * @box: a #BreaffyFlowBox
 ...
 *     always be a #BreaffyFlowBoxChild or %NULL in case no child widget
 *     with the given index exists.
 */
BreaffyFlowBoxChild *
breaffy_flow_box_get_child_at_index (BreaffyFlowBox *box,
                                 gint        idx)
{
  GSequenceIter *iter;

  g_return_val_if_fail (BREAFFY_IS_FLOW_BOX (box), NULL);
...
/**
 * breaffy_flow_box_get_child_at_pos:
 * @box: a #BreaffyFlowBox
 ...
 */
BreaffyFlowBoxChild *
breaffy_flow_box_get_child_at_pos (BreaffyFlowBox *box,
                               gint        x,
                               gint        y)
{
  GtkWidget *child;
  ...
  for (iter = g_sequence_get_begin_iter (BOX_PRIV (box)->children);
       ...
       iter = g_sequence_iter_next (iter))
    {
      ...
      if (gdk_rectangle_contains_point (&allocation, x, y))
        return BREAFFY_FLOW_BOX_CHILD (child);
    }
...
/**
 * breaffy_flow_box_set_hadjustment:
 * @box: a #BreaffyFlowBox
 ...
 * breaffy_flow_box_set_vadjustment()for setting the vertical
 * adjustment.
 *
 ...
 */
void
breaffy_flow_box_set_hadjustment (BreaffyFlowBox    *box,
                              GtkAdjustment *adjustment)
{
  BreaffyFlowBoxPrivate *priv;
...
/**
 * breaffy_flow_box_set_vadjustment:
 * @box: a #BreaffyFlowBox
 * @adjustment: an adjustment which should be adjusted
 *    when the focus is moved among the descendents of @container
 *
 ...
 * for a typical way of obtaining the adjustment, and
 * breaffy_flow_box_set_hadjustment()for setting the horizontal
 * adjustment.
 *
 * The adjustments have to be in pixel units and in the same
 * coordinate system as the allocation for immediate children
 ...
 */
void
breaffy_flow_box_set_vadjustment (BreaffyFlowBox    *box,
                              GtkAdjustment *adjustment)
{
  BreaffyFlowBoxPrivate *priv;
...
static void
breaffy_flow_box_check_model_compat (BreaffyFlowBox *box)
{
  BreaffyFlowBoxPrivate *priv = BOX_PRIV (box);

  if (priv->bound_model &&
      (priv->sort_func || priv->filter_func))
    g_warning ("BreaffyFlowBox with a model will ignore sort and filter functions");
}

/**
 * breaffy_flow_box_bind_model:
 * @box: a #BreaffyFlowBox
 ...
 * breaffy_flow_box_insert() or gtk_container_add()) while @box is bound to a
 * model.
 *
 * Note that using a model is incompatible with the filtering and sorting
 * functionality in BreaffyFlowBox. When using a model, filtering and sorting
 * should be implemented by the model.
 */
void
breaffy_flow_box_bind_model (BreaffyFlowBox                 *box,
                         GListModel                 *model,
                         BreaffyFlowBoxCreateWidgetFunc  create_widget_func,
                         gpointer                    user_data,
                         GDestroyNotify              user_data_free_func)
{
  ...
  if (priv->bound_model)
    {
      ...
      g_signal_handlers_disconnect_by_func (priv->bound_model, breaffy_flow_box_bound_model_changed, box);
      g_clear_object (&priv->bound_model);
    }

  breaffy_flow_box_forall (GTK_CONTAINER (box), (GtkCallback) gtk_widget_destroy, NULL);
  ...
  breaffy_flow_box_check_model_compat (box);

  g_signal_connect (priv->bound_model, "items-changed", G_CALLBACK (breaffy_flow_box_bound_model_changed), box);
  breaffy_flow_box_bound_model_changed (model, 0, 0, g_list_model_get_n_items (model), box);
}

/* Setters and getters {{{2 */

/**
 * breaffy_flow_box_get_homogeneous:
 * @box: a #BreaffyFlowBox
 *
 ...
 */
gboolean
breaffy_flow_box_get_homogeneous (BreaffyFlowBox *box)
{
  g_return_val_if_fail (BREAFFY_IS_FLOW_BOX (box), FALSE);

  return BOX_PRIV (box)->homogeneous;
}

/**
 * breaffy_flow_box_set_homogeneous:
 * @box: a #BreaffyFlowBox
 * @homogeneous: %TRUE to create equal allotments,
 *   %FALSE for variable allotments
 *
 * Sets the #BreaffyFlowBox:homogeneous property of @box, controlling
 * whether or not all children of @box are given equal space
 * in the box.
 */
void
breaffy_flow_box_set_homogeneous (BreaffyFlowBox *box,
                              gboolean    homogeneous)
{
  g_return_if_fail (BREAFFY_IS_FLOW_BOX (box));
...
/**
 * breaffy_flow_box_set_row_spacing:
 * @box: a #BreaffyFlowBox
 * @spacing: the spacing to use
 *
 * Sets the vertical space to add between children.
 * See the #BreaffyFlowBox:row-spacing property.
 */
void
breaffy_flow_box_set_row_spacing (BreaffyFlowBox *box,
                              guint       spacing)
{
  g_return_if_fail (BREAFFY_IS_FLOW_BOX (box));
...
/**
 * breaffy_flow_box_get_row_spacing:
 * @box: a #BreaffyFlowBox
 *
 ...
 */
guint
breaffy_flow_box_get_row_spacing (BreaffyFlowBox *box)
{
  g_return_val_if_fail (BREAFFY_IS_FLOW_BOX (box), FALSE);
...
/**
 * breaffy_flow_box_set_column_spacing:
 * @box: a #BreaffyFlowBox
 * @spacing: the spacing to use
 *
 * Sets the horizontal space to add between children.
 * See the #BreaffyFlowBox:column-spacing property.
 */
void
breaffy_flow_box_set_column_spacing (BreaffyFlowBox *box,
                                 guint       spacing)
{
  g_return_if_fail (BREAFFY_IS_FLOW_BOX (box));
...
/**
 * breaffy_flow_box_get_column_spacing:
 * @box: a #BreaffyFlowBox
 *
 ...
 */
guint
breaffy_flow_box_get_column_spacing (BreaffyFlowBox *box)
{
  g_return_val_if_fail (BREAFFY_IS_FLOW_BOX (box), FALSE);
...
/**
 * breaffy_flow_box_set_min_children_per_line:
 * @box: a #BreaffyFlowBox
 ...
 */
void
breaffy_flow_box_set_min_children_per_line (BreaffyFlowBox *box,
                                        guint       n_children)
{
  g_return_if_fail (BREAFFY_IS_FLOW_BOX (box));
...
/**
 * breaffy_flow_box_get_min_children_per_line:
 * @box: a #BreaffyFlowBox
 *
 ...
 */
guint
breaffy_flow_box_get_min_children_per_line (BreaffyFlowBox *box)
{
  g_return_val_if_fail (BREAFFY_IS_FLOW_BOX (box), FALSE);
...
/**
 * breaffy_flow_box_set_max_children_per_line:
 * @box: a #BreaffyFlowBox
 ...
 */
void
breaffy_flow_box_set_max_children_per_line (BreaffyFlowBox *box,
                                        guint       n_children)
{
  g_return_if_fail (BREAFFY_IS_FLOW_BOX (box));
...
/**
 * breaffy_flow_box_get_max_children_per_line:
 * @box: a #BreaffyFlowBox
 *
 ...
 */
guint
breaffy_flow_box_get_max_children_per_line (BreaffyFlowBox *box)
{
  g_return_val_if_fail (BREAFFY_IS_FLOW_BOX (box), FALSE);
...
/**
 * breaffy_flow_box_set_activate_on_single_click:
 * @box: a #BreaffyFlowBox
 ...
 */
void
breaffy_flow_box_set_activate_on_single_click (BreaffyFlowBox *box,
                                           gboolean    single)
{
  g_return_if_fail (BREAFFY_IS_FLOW_BOX (box));
...
/**
 * breaffy_flow_box_get_activate_on_single_click:
 * @box: a #BreaffyFlowBox
 *
 ...
 */
gboolean
breaffy_flow_box_get_activate_on_single_click (BreaffyFlowBox *box)
{
  g_return_val_if_fail (BREAFFY_IS_FLOW_BOX (box), FALSE);
...
static void
breaffy_flow_box_set_accept_unpaired_release (BreaffyFlowBox *box,
                                          gboolean    accept)
{
...
/**
 * breaffy_flow_box_get_selected_children:
 * @box: a #BreaffyFlowBox
 *
 * Creates a list of all selected children.
 *
 * Returns: (element-type BreaffyFlowBoxChild) (transfer container):
 *     A #GList containing the #GtkWidget for each selected child.
 *     Free with g_list_free() when done.
 */
GList *
breaffy_flow_box_get_selected_children (BreaffyFlowBox *box)
{
  BreaffyFlowBoxChild *child;
  GSequenceIter *iter;
  GList *selected = NULL;
 ...
 */
void
breaffy_flow_box_select_child (BreaffyFlowBox      *box,
                           BreaffyFlowBoxChild *child)
{
  g_return_if_fail (BREAFFY_IS_FLOW_BOX (box));
  g_return_if_fail (BREAFFY_IS_FLOW_BOX_CHILD (child));
...
/**
 * breaffy_flow_box_unselect_child:
 * @box: a #BreaffyFlowBox
 * @child: a child of @box
 *
 ...
 */
void
breaffy_flow_box_unselect_child (BreaffyFlowBox      *box,
                             BreaffyFlowBoxChild *child)
{
  g_return_if_fail (BREAFFY_IS_FLOW_BOX (box));
  g_return_if_fail (BREAFFY_IS_FLOW_BOX_CHILD (child));

  breaffy_flow_box_unselect_child_internal (box, child);
}

/**
 * breaffy_flow_box_select_all:
 * @box: a #BreaffyFlowBox
 *
 ...
 */
void
breaffy_flow_box_select_all (BreaffyFlowBox *box)
{
  g_return_if_fail (BREAFFY_IS_FLOW_BOX (box));
  ...
  if (g_sequence_get_length (BOX_PRIV (box)->children) > 0)
    {
      breaffy_flow_box_select_all_between (box, NULL, NULL, FALSE);
      g_signal_emit (box, signals[SELECTED_CHILDREN_CHANGED], 0);
    }
}

/**
 * breaffy_flow_box_unselect_all:
 * @box: a #BreaffyFlowBox
 *
 ...
 */
void
breaffy_flow_box_unselect_all (BreaffyFlowBox *box)
{
  gboolean dirty = FALSE;
  ...
  dirty = breaffy_flow_box_unselect_all_internal (box);
...
/**
 * BreaffyFlowBoxForeachFunc:
 * @box: a #BreaffyFlowBox
 * @child: a #BreaffyFlowBoxChild
 * @user_data: (closure): user data
 *
 * A function used by breaffy_flow_box_selected_foreach().
 * It will be called on every selected child of the @box.
 */

/**
 * breaffy_flow_box_selected_foreach:
 * @box: a #BreaffyFlowBox
 ...
 */
void
breaffy_flow_box_selected_foreach (BreaffyFlowBox            *box,
...
/**
 * breaffy_flow_box_set_selection_mode:
 * @box: a #BreaffyFlowBox
 ...
 */
void
breaffy_flow_box_set_selection_mode (BreaffyFlowBox       *box,
...
/**
 * breaffy_flow_box_get_selection_mode:
 * @box: a #BreaffyFlowBox
 *
 ...
 */
GtkSelectionMode
breaffy_flow_box_get_selection_mode (BreaffyFlowBox *box)
{
  g_return_val_if_fail (BREAFFY_IS_FLOW_BOX (box), GTK_SELECTION_SINGLE);
...
/**
 * BreaffyFlowBoxFilterFunc:
 * @child: a #BreaffyFlowBoxChild that may be filtered
 * @user_data: (closure): user data
 *
...
/**
 * breaffy_flow_box_set_filter_func:
 * @box: a #BreaffyFlowBox
 ...
 * breaffy_flow_box_child_changed()) or when breaffy_flow_box_invalidate_filter()
 * is called.
 *
 * Note that using a filter function is incompatible with using a model
 * (see breaffy_flow_box_bind_model()).
 */
void
breaffy_flow_box_set_filter_func (BreaffyFlowBox           *box,
                              BreaffyFlowBoxFilterFunc  filter_func,
                              gpointer              user_data,
                              GDestroyNotify        destroy)
{
  ...
  breaffy_flow_box_apply_filter_all (box);
}

/**
 * breaffy_flow_box_invalidate_filter:
 * @box: a #BreaffyFlowBox
 *
 ...
 */
void
breaffy_flow_box_invalidate_filter (BreaffyFlowBox *box)
{
  g_return_if_fail (BREAFFY_IS_FLOW_BOX (box));
...
/**
 * breaffy_flow_box_set_sort_func:
 * @box: a #BreaffyFlowBox
 ...
 * breaffy_flow_box_child_changed()) and when breaffy_flow_box_invalidate_sort()
 * is called.
 *
 ...
 */
void
breaffy_flow_box_set_sort_func (BreaffyFlowBox         *box,
                            BreaffyFlowBoxSortFunc  sort_func,
                            gpointer            user_data,
                            GDestroyNotify      destroy)
{
  ...
  breaffy_flow_box_invalidate_sort (box);
}

static gint
breaffy_flow_box_sort (BreaffyFlowBoxChild *a,
                   BreaffyFlowBoxChild *b,
                   BreaffyFlowBox      *box)
{
  BreaffyFlowBoxPrivate *priv = BOX_PRIV (box);
...
static void
breaffy_flow_box_css_node_foreach (gpointer data,
                               gpointer user_data)
{
...
/**
 * breaffy_flow_box_invalidate_sort:
 * @box: a #BreaffyFlowBox
 *
 ...
 */
void
breaffy_flow_box_invalidate_sort (BreaffyFlowBox *box)
{
  BreaffyFlowBoxPrivate *priv;
  ...
  if (priv->sort_func != NULL)
    {
      g_sequence_sort (priv->children, (GCompareDataFunc)breaffy_flow_box_sort, box);
      g_sequence_foreach (priv->children, breaffy_flow_box_css_node_foreach, &previous);
      gtk_widget_queue_resize (GTK_WIDGET (box));
    }
}
