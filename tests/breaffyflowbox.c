                        ...
                        GtkFlowBox  *flowbox)
{
  ...
  breaffy_flow_box_set_selection_mode (flowbox, mode);
}
                     ...
                     GtkFlowBox *flowbox)
{
  ...
  breaffy_flow_box_set_min_children_per_line (flowbox, length);
}
                         ...
                         GtkFlowBox *flowbox)
{
  ...
  breaffy_flow_box_set_max_children_per_line (flowbox, length);
}
                 ...
                 gpointer       data)
{
  ...
  if (orientation == GTK_ORIENTATION_HORIZONTAL)
    breaffy_flow_box_set_column_spacing (the_flowbox, state);
  else
    breaffy_flow_box_set_row_spacing (the_flowbox, state);
}
                     ...
                     GtkFlowBox      *flowbox)
{
  ...
  breaffy_flow_box_set_homogeneous (flowbox, state);
}
...
on_selected_children_changed (GtkFlowBox *self)
{
  ...
  //breaffy_flow_box_selected_foreach (self, selection_foreach, NULL);
}
...
filter_func (GtkFlowBoxChild *child, gpointer user_data)
{
  ...
  index = breaffy_flow_box_child_get_index (child);
                ...
                GtkFlowBox      *flowbox)
{
  ...
  if (state)
    breaffy_flow_box_set_filter_func (flowbox, filter_func, NULL, NULL);
  else
    breaffy_flow_box_set_filter_func (flowbox, NULL, NULL, NULL);
}
              ...
              GtkFlowBox      *flowbox)
{
  ...
  if (state)
    breaffy_flow_box_set_sort_func (flowbox, sort_func, NULL, NULL);
  else
    breaffy_flow_box_set_sort_func (flowbox, NULL, NULL, NULL);
}
...
create_window (void)
{
  ...
  breaffy_flow_box_set_column_spacing (BREAFFY_FLOW_BOX (flowbox), INITIAL_CSPACING);
  breaffy_flow_box_set_row_spacing (BREAFFY_FLOW_BOX (flowbox), INITIAL_RSPACING);
  breaffy_flow_box_set_min_children_per_line (BREAFFY_FLOW_BOX (flowbox), INITIAL_MINIMUM_LENGTH);
  breaffy_flow_box_set_max_children_per_line (BREAFFY_FLOW_BOX (flowbox), INITIAL_MAXIMUM_LENGTH);
  gtk_widget_show (flowbox);
  gtk_container_add (GTK_CONTAINER (swindow), flowbox);

  breaffy_flow_box_set_hadjustment (BREAFFY_FLOW_BOX (flowbox),
                                gtk_scrolled_window_get_hadjustment (GTK_SCROLLED_WINDOW (swindow)));
  breaffy_flow_box_set_vadjustment (BREAFFY_FLOW_BOX (flowbox),
  ...
  populate_items (BREAFFY_FLOW_BOX (flowbox));
